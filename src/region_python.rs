        ...
        match self {
            PyRegionExpr::Halfspace(hs) => {
                ...
                Python::with_gil(|py| {
                    ...
                    if hs.is_above {
                        crate::region::RegionExpr::Halfspace(crate::region::HalfspaceType::Above(arc_surface))
                    } else {
                        crate::region::RegionExpr::Halfspace(crate::region::HalfspaceType::Below(arc_surface))
                    }
                })
            }
            PyRegionExpr::Union(a, b) => {
                crate::region::RegionExpr::Union(
                    Box::new(a.to_region_expr()),
                    Box::new(b.to_region_expr()),
                )
            }
            PyRegionExpr::Intersection(a, b) => {
                crate::region::RegionExpr::Intersection(
                    Box::new(a.to_region_expr()),
                    Box::new(b.to_region_expr()),
                )
            }
...
impl PyRegion {
    ...
    fn __and__(&self, other: &PyAny) -> PyResult<PyRegion> {
        let expr = if let Ok(other_region) = other.extract::<PyRef<PyRegion>>() {
            PyRegionExpr::Intersection(Box::new(self.expr.clone()), Box::new(other_region.expr.clone()))
        } else if let Ok(other_halfspace) = other.extract::<PyRef<PyHalfspace>>() {
            PyRegionExpr::Intersection(Box::new(self.expr.clone()), Box::new(PyRegionExpr::Halfspace(other_halfspace.clone())))
        } else {
    ...
    fn __or__(&self, other: &PyAny) -> PyResult<PyRegion> {
        let expr = if let Ok(other_region) = other.extract::<PyRef<PyRegion>>() {
            PyRegionExpr::Union(Box::new(self.expr.clone()), Box::new(other_region.expr.clone()))
        } else if let Ok(other_halfspace) = other.extract::<PyRef<PyHalfspace>>() {
            PyRegionExpr::Union(Box::new(self.expr.clone()), Box::new(PyRegionExpr::Halfspace(other_halfspace.clone())))
        } else {
            return Err(pyo3::exceptions::PyTypeError::new_err("Operand must be PyRegion or PyHalfspace"));
        };
        let region = crate::region::Region {
...
impl PyHalfspace {
    ...
    fn __and__(&self, other: &PyAny) -> PyResult<PyRegion> {
        let expr = if let Ok(other_halfspace) = other.extract::<PyRef<PyHalfspace>>() {
            PyRegionExpr::Intersection(Box::new(PyRegionExpr::Halfspace(self.clone())), Box::new(PyRegionExpr::Halfspace(other_halfspace.clone())))
        } else if let Ok(other_region) = other.extract::<PyRef<PyRegion>>() {
            PyRegionExpr::Intersection(Box::new(PyRegionExpr::Halfspace(self.clone())), Box::new(other_region.expr.clone()))
        } else {
            return Err(pyo3::exceptions::PyTypeError::new_err("Operand must be PyRegion or PyHalfspace"));
        };
        let region = crate::region::Region {
    ...
    fn __or__(&self, other: &PyAny) -> PyResult<PyRegion> {
        let expr = if let Ok(other_halfspace) = other.extract::<PyRef<PyHalfspace>>() {
            PyRegionExpr::Union(Box::new(PyRegionExpr::Halfspace(self.clone())), Box::new(PyRegionExpr::Halfspace(other_halfspace.clone())))
        } else if let Ok(other_region) = other.extract::<PyRef<PyRegion>>() {
            PyRegionExpr::Union(Box::new(PyRegionExpr::Halfspace(self.clone())), Box::new(other_region.expr.clone()))
        } else {
            return Err(pyo3::exceptions::PyTypeError::new_err("Operand must be PyRegion or PyHalfspace"));
        };
        let region = crate::region::Region {
